...
<head>
    ...
    <title>Ciudadanos| Sistema de registro de enfermedades</title>
</head>
        ...
        <div class="card shadow-lg p-3 mb-3 bg-white rounded mt-4" style="max-width: 800px; margin: 0 auto;">
            <div class="card-body">
                <div class="card-title pb-2 text-center">
                    <h4 class="text-secondary">Lista de ciudadanos con enfermedadaes asociadas</h4>
                </div>
                <div class="table-responsive">
                    <table class="table table-striped text-center table-hover">
                        ...
                        <tbody>
                            <?php foreach ($this->model->listar_ciudadanos() as $row) :  ?>
                                <tr>
                                    <th scope="row"><?= $row['id_ciudadano'] ?></th>
                                    <td><?= $row['numero_identificacion'] ?></td>
                                    <td><?= $row['nombres'] ?></td>
                                    <td><?= $row['apellidos'] ?></td>
                                    <td>
                                        <?php foreach ($this->model->listar_ciudadanos_enfermedadaes($row['id_ciudadano']) as $row) :  ?>
                                            <?= $row['nombre'] . '<br>' ?>
                                        <?php endforeach; ?>
                                    </td>
                                </tr>
